...
- **Estructura Mientras/`while`**: representación equivalente en diagrama de flujo, pseudocódigo (`Mientras (condicion) Haga ... Fin_Mientras`) y Python (`while (condicion): ...`).
...
- **Ciclos infinitos**: una condición que nunca se vuelve falsa por sí sola — por error (se olvida actualizar la variable de control) o intencional (no hay una condición de parada natural al escribir el ciclo, y se recurre a la ruptura de ciclos para detenerlo desde adentro).
- **Ruptura de ciclos**: dos formas de terminar un ciclo antes de que su condición se vuelva falsa "por las buenas" — la forma estructurada (una bandera dentro de la condición del `Mientras`/`while`) y la instrucción directa `Romper`/`break`, que interrumpe de inmediato sin esperar a que se reevalúe la condición.
- **Ejemplo 8** (buscar si un cero aparece entre `N` números leídos): resuelto en dos formas equivalentes que se comparan lado a lado — con la bandera `encontrado` dentro de la condición del `while` (`ciclos_ejemplo8_sin_break.py`), y con la condición simplificada a `i < N` y un `break` al encontrar el valor (`ciclos_ejemplo8_con_break.py`).
- **Seis casos de uso comunes de `break`**, cada uno con su propio ejemplo: validación de datos de entrada hasta que sean válidos (ejemplo 9, edad entre 0 y 120), menú interactivo hasta que el usuario elige salir (ejemplo 10), lectura hasta un centinela sin saber cuántos datos vendrán (ejemplo 11, suma de números terminada en -1), detección de un dato inválido que interrumpe el ciclo de inmediato (ejemplo 12), un límite de intentos combinando dos motivos de parada — éxito o intentos agotados (ejemplo 13, validación de clave) — y ciclos anidados, donde `break` solo corta el ciclo interno que lo contiene (ejemplo 14, números primos).
...
| [codigo/ciclos_ejemplo8_sin_break.py](codigo/ciclos_ejemplo8_sin_break.py) | Ejemplo 8, forma estructurada: busca si un cero aparece entre `N` números leídos, deteniendo el ciclo con la bandera `encontrado` dentro de la condición del `while`. |
| [codigo/ciclos_ejemplo8_con_break.py](codigo/ciclos_ejemplo8_con_break.py) | Ejemplo 8, forma con `break`: mismo problema, con la condición simplificada a `i < N` y un `break` al encontrar el valor buscado. |
| [codigo/ciclos_ejemplo9.py](codigo/ciclos_ejemplo9.py) | Ejemplo 9 (validación de datos de entrada): `while True` que solo termina con `break` cuando se ingresa una edad válida (0–120). |
| [codigo/ciclos_ejemplo10.py](codigo/ciclos_ejemplo10.py) | Ejemplo 10 (menú interactivo): `while True` con un menú de tres opciones; `break` termina el ciclo solo cuando el usuario elige "Salir". |
| [codigo/ciclos_ejemplo11.py](codigo/ciclos_ejemplo11.py) | Ejemplo 11 (lectura hasta un centinela): suma números ingresados uno a uno hasta que se lee `-1`, cortando con `break` antes de sumarlo. |
| [codigo/ciclos_ejemplo12.py](codigo/ciclos_ejemplo12.py) | Ejemplo 12 (detección de error): lee `N` datos y corta con `break` apenas aparece uno negativo, marcando `huboError`. |
| [codigo/ciclos_ejemplo13.py](codigo/ciclos_ejemplo13.py) | Ejemplo 13 (límite de intentos): valida una clave con máximo 3 intentos — `break` controla el acierto, el `while` controla el agotamiento de intentos. |
| [codigo/ciclos_ejemplo14.py](codigo/ciclos_ejemplo14.py) | Ejemplo 14 (ciclos anidados): determina los números primos hasta un límite dado; `break` corta solo el ciclo `while` interno, sin afectar el `for` externo. |
